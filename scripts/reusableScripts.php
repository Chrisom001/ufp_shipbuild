        ...
        if($shipItem->$check == 1){
            if($itemType == "console"){
                if($shipItem->equipmentType == $type){
                    $consoleData = json_decode(getEquipmentNameById($shipItem->equipmentTypeID)) . " (" . $shipItem->equipmentType . ") ". " MK " . json_decode(getItemTierById($shipItem->itemTierID));
...
function getBridgeOfficerSlots($shipID){
    ...
    for($i = 0; $i < count($bridgeOfficerSlots); $i++){
        ...
        $result .= "<p>Slot " . ($i+1) .":". $bridgeOfficerSlots[$i]->slotRank . " " . $bridgeOfficerSlots[$i]->slotType . "</p>";
        for($j =0; $j < 3; $j++){
